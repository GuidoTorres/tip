...
const event: PaymentWebhookEvent = { eventId: "evt-1", providerPaymentId: "pay-1", status: "confirmed", gatewayFeeMinor: 80, occurredAt: "2026-08-12T20:00:00.000Z" };
...
function setup(overrides?: { verified?: boolean; newlyProcessed?: boolean; event?: PaymentWebhookEvent }) {
  ...
  const provider: PaymentProvider = {
    name: "mock", createPayment: vi.fn(), getPaymentStatus: vi.fn(), createPayout: vi.fn(), getPayoutStatus: vi.fn(),
    verifyWebhook: vi.fn().mockReturnValue(overrides?.verified ?? true), parseWebhook: vi.fn().mockReturnValue(parsed),
  };
...
describe("processPaymentWebhook", () => {
  it("confirma server-side y envía una sola notificación lógica", async () => {
    const deps = setup();
    const result = await processPaymentWebhook("raw", "signature", deps);
  ...
  it("rechaza una firma inválida sin efectos", async () => {
    ...
    await expect(processPaymentWebhook("raw", "bad", deps)).rejects.toThrow("invalid_webhook");
  ...
  it("mantiene confirmado el webhook cuando falla Web Push", async () => {
    ...
    await expect(processPaymentWebhook("raw", "signature", deps)).resolves.toEqual({ ok: true, duplicate: false, status: "confirmed", pushFailed: true });
    expect(deps.repository.confirm).toHaveBeenCalledTimes(1);
  });
