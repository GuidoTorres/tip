...
import { getMercadoPagoRegion, isMercadoPagoCountry } from "./mercadopago-regions";
...
export class MercadoPagoCredentialManager {
  ...
  async findByAccountId(accountId: string, country?: string) {
    ...
    if (!credentials.refreshToken || !country || !isMercadoPagoCountry(country)) throw new Error("mercadopago_reauthorization_required");
    const token = await this.client.refreshToken(getMercadoPagoRegion(country, this.env), credentials.refreshToken);
    const refreshed = {
      accountId,
