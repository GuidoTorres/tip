...
const es = {
  ...
  refunds: {
    ...
    sections: [
      { title: "Apoyo voluntario", paragraphs: ["El fan elige libremente el perfil y el monto. Después de la confirmación, TipMe no ofrece una cancelación automática porque el proveedor activo ya procesó el cobro."] },
      { title: "Solicitud voluntaria", paragraphs: ["El fan puede solicitar ayuda indicando el código de operación. Si corresponde una devolución, debe tramitarse sobre el pago original mediante el proveedor. Nunca se devuelve mediante una transferencia externa ni a un medio distinto."] },
...
const en: typeof es = {
  ...
  refunds: {
    ...
    sections: [
      ...
      { title: "Voluntary request", paragraphs: ["A fan may request help using the transaction code. When a refund applies, it must be processed against the original payment through the provider. It must never be sent through an external transfer or to a different payment method."] },
