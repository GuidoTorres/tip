...
export function DashboardProfileHeader({ name, avatarUrl }: { name: string; avatarUrl: string | null }) {
  ...
  return <div className="flex min-w-0 items-center gap-3">
    {avatarUrl
      ...
      : <div aria-label={`Inicial de ${name}`} className="grid size-12 shrink-0 place-items-center rounded-2xl bg-accent-strong text-lg font-bold text-on-accent">{initial}</div>}
