  ...
  return <div className={separateCards ? "space-y-3" : "overflow-hidden rounded-2xl border border-border bg-surface"}>
    {tips.map((tip, index) => {
      ...
      return <Link key={tip.id} href={`/dashboard/tips/${tip.id}`} data-tip-card={separateCards ? true : undefined} className={`flex gap-3 p-3.5 hover:bg-surface-soft ${separateCards ? "rounded-2xl border border-border bg-surface" : index ? "border-t border-border" : ""}`}>
        <div className="grid size-10 shrink-0 place-items-center rounded-xl bg-surface-soft font-bold text-accent-strong">{tip.anonymous ? "A" : (tip.payer_name ?? "A").charAt(0).toUpperCase()}</div>
        <div className="min-w-0 flex-1">
          ...
          <p className={`mt-1.5 flex items-center gap-1 text-xs font-semibold ${status.tone === "success" ? "text-success" : status.tone === "danger" ? "text-accent-strong" : "text-warning"}`}><StatusIcon weight="fill" />{status.label} · {formatDistanceToNow(tip.created_at)}</p>
        </div>
      </Link>;
    })}
  </div>;
