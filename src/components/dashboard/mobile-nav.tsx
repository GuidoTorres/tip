...
import { GearSix, Heart, House } from "@phosphor-icons/react";
...
export function MobileNav() {
  ...
  return <nav className="fixed inset-x-0 bottom-0 z-20 grid grid-cols-3 border-t border-border bg-surface/95 px-[max(0.75rem,env(safe-area-inset-left))] pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur md:hidden" aria-label="Dashboard">{items.map(({ href, label, icon: Icon }) => {
