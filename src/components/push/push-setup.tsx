...
export function PushSetup({ vapidPublicKey, compact = false, header = false }: { vapidPublicKey: string; compact?: boolean; header?: boolean }) {
  ...
  if (state === "install-ios") return compact
    ? <p className="flex items-center gap-2 text-sm font-semibold text-muted"><span className="grid size-11 shrink-0 place-items-center rounded-full bg-surface-soft text-accent-strong"><ShareNetwork size={21} /></span><span>Añade TipMe a inicio para activarlas</span></p>
    : <div className="rounded-2xl bg-surface-soft p-5"><ShareNetwork size={26} className="text-accent-strong" /><h3 className="mt-3 font-semibold">Recibe tus tips al instante</h3><ol className="mt-3 space-y-2 text-sm text-muted"><li>1. Pulsa Compartir.</li><li>2. Selecciona “Añadir a pantalla de inicio”.</li><li>3. Abre TipMe desde el icono.</li><li>4. Activa las notificaciones.</li></ol></div>;
  if (state === "active") return compact
    ? <p role="status" aria-label="Notificaciones activadas" title="Notificaciones activadas" className="grid size-11 place-items-center rounded-full bg-accent-strong text-on-accent"><Bell size={21} weight="fill" /></p>
  ...
  return <div>{state === "error" && <p className="mb-3 text-sm text-accent-strong">No pudimos activar las notificaciones. Revisa tu conexión.</p>}<button type="button" onClick={enable} className="pressable flex min-h-14 w-full items-center justify-center gap-2 rounded-full bg-accent-strong px-6 py-4 font-bold text-on-accent hover:bg-accent-pressed"><Bell size={21} weight="fill" /> Activar notificaciones</button></div>;
}
