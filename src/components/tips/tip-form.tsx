...
export function TipForm({ username, currency, locale, checkoutFeeBps = 0, checkoutFixedFeeMinor = 0 }: {
  username: string;
  currency: Currency;
  locale: Locale;
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/tips", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({
        ...
        message: String(form.get("message") || ""), anonymous, legalAccepted, coverProcessing,
