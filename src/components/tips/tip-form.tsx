...
export function TipForm({ username, currency, locale, checkoutFeeBps = 0, checkoutFixedFeeMinor = 0 }: {
  ...
  useEffect(() => {
    ...
    fetch(`/api/payments/checkout-config?username=${encodeURIComponent(username)}`, {
      ...
      if (!response.ok || !data || !["embedded", "redirect"].includes(data.kind)) throw new Error("checkout_unavailable");
  ...
  return <form ref={formRef} onSubmit={submitRedirect} className="mt-8">
    <fieldset disabled={submitting || checkoutLocked}>
      ...
      <div className="mt-6 space-y-4">
        ...
        <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-border px-4 py-3"><input type="checkbox" checked={coverProcessing} onChange={(event) => setCoverProcessing(event.target.checked)} className="mt-0.5 size-5 shrink-0 accent-accent" /><span><span className="block text-sm font-semibold">{locale === "es" ? "Ayudar a cubrir el procesamiento" : "Help cover processing"}</span><span className="mt-0.5 block text-xs leading-relaxed text-muted">{locale === "es" ? "Aporte voluntario estimado; PayPal determina la comisión real." : "Optional estimate; PayPal determines the actual fee."}</span></span></label>
