...
export default async function AdminCreatorsPage() {
  ...
  return <AdminList title="Perfiles" empty="No hay perfiles.">{data?.map((item) => <div key={item.id} className="grid gap-1 p-4 sm:grid-cols-[1fr_1fr_auto]"><strong>{item.public_name ?? "Sin nombre"}</strong><span className="text-sm text-muted">@{item.username ?? "sin-username"}</span><span className="text-sm text-muted">{item.country ?? "--"} · {item.preferred_currency} · {item.onboarding_completed ? "Listo" : "Onboarding"}</span></div>)}</AdminList>;
}
