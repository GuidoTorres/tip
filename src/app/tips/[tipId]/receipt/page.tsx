...
export default async function ReceiptPage({ params, searchParams }: { params: Promise<{ tipId: string }>; searchParams: Promise<{ token?: string }> }) {
  ...
  if (!data) notFound();
  const receipt = data as unknown as { id: string; status: TipStatus; amount_minor: number; currency: Currency; provider: string; message: string | null; profiles: { public_name: string | null; username: string } | null };
