...
export default async function ReceiptPage({ params, searchParams }: { params: Promise<{ tipId: string }>; searchParams: Promise<{ token?: string }> }) {
  ...
  const { data } = await createAdminSupabaseClient().from("tips").select("id,status,base_amount_minor,processing_support_minor,amount_minor,currency,provider,message,profiles!tips_creator_id_fkey(public_name,username)").eq("id", tipId).single();
  if (!data) notFound();
  const receipt = data as unknown as { id: string; status: TipStatus; base_amount_minor: number | null; processing_support_minor: number; amount_minor: number; currency: Currency; provider: string; message: string | null; profiles: { public_name: string | null; username: string } | null };
