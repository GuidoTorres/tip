...
export default async function OnboardingPage({ searchParams }: { searchParams: Promise<{ step?: string; error?: string }> }) {
  ...
  const { data: profile } = await supabase.from("profiles").select("public_name,username,bio,country,preferred_currency,locale").eq("id", user.id).single();
  const env = getPublicEnv();
  const username = profile?.username ?? "username";
  const publicUrl = `${env.NEXT_PUBLIC_APP_URL.replace(/\/$/, "")}/${username}`;

  return <main className="min-h-[100dvh] px-4 py-8"><div className="mx-auto max-w-lg"><div className="flex items-center justify-between"><span className="text-xl font-bold tracking-[-0.04em]">TipMe<span className="text-accent">.</span></span><span className="text-sm text-muted">{step} de 4</span></div><div className="mt-4 h-1 overflow-hidden rounded-full bg-surface-soft"><div className="h-full bg-accent transition-[width]" style={{ width: `${Math.min(4, Math.max(1, Number(step))) * 25}%` }} /></div>
    <section className="mt-8 rounded-2xl border border-border bg-surface p-6 shadow-[var(--shadow)] sm:p-8">
      ...
      {step === "1" && <><UserCircle size={32} className="text-accent" weight="fill" /><h1 className="mt-4 text-3xl font-semibold tracking-[-0.04em]">Tu página pública</h1><p className="mt-2 text-muted">Así te verán las personas que quieran apoyarte.</p><form action={saveOnboardingProfile} className="mt-7 space-y-5"><label className="block text-sm font-semibold">Nombre público<input className={inputClass} name="publicName" required maxLength={80} defaultValue={profile?.public_name ?? ""} /></label><label className="block text-sm font-semibold">Username<div className="mt-2 flex min-h-12 items-center rounded-xl border border-border bg-background px-4"><span className="text-muted">tipme.pro/</span><input name="username" required minLength={3} maxLength={30} defaultValue={profile?.username ?? ""} className="min-w-0 flex-1 bg-transparent outline-none" /></div></label><label className="block text-sm font-semibold">Foto<input className="mt-2 block w-full text-sm text-muted file:mr-3 file:rounded-full file:border-0 file:bg-surface-soft file:px-4 file:py-3 file:font-semibold" type="file" name="avatar" accept="image/jpeg,image/png,image/webp,image/avif" /></label><label className="block text-sm font-semibold">Descripción<textarea className={`${inputClass} min-h-24 py-3`} name="bio" maxLength={180} defaultValue={profile?.bio ?? ""} /></label><div className="grid grid-cols-2 gap-3"><label className="block text-sm font-semibold">País<select className={inputClass} name="country" defaultValue={profile?.country ?? "PE"}><option value="PE">Perú</option><option value="CO">Colombia</option><option value="BR">Brasil</option><option value="CL">Chile</option><option value="AR">Argentina</option><option value="US">Estados Unidos</option><option value="ES">España</option></select></label><label className="block text-sm font-semibold">Moneda<select className={inputClass} name="currency" defaultValue={profile?.preferred_currency ?? "USD"}>{["USD","EUR","PEN","COP","BRL","CLP","ARS"].map((currency) => <option key={currency}>{currency}</option>)}</select></label></div><input type="hidden" name="locale" value={profile?.locale ?? "es"} /><SubmitButton label="Continuar" /></form></>}
      {step === "2" && <><Bank size={32} className="text-accent" weight="fill" /><h1 className="mt-4 text-3xl font-semibold tracking-[-0.04em]">Método de retiro</h1><p className="mt-2 text-muted">Para el piloto usaremos una cuenta bancaria simulada.</p><form action={saveMockPayoutAccount} className="mt-7 space-y-5"><label className="block text-sm font-semibold">Banco<input className={inputClass} name="bankName" required maxLength={80} defaultValue="Banco Demo" /></label><label className="block text-sm font-semibold">Últimos 4 dígitos<input className={inputClass} name="last4" required inputMode="numeric" pattern="[0-9]{4}" maxLength={4} defaultValue="4821" /></label><input type="hidden" name="country" value={profile?.country ?? "PE"} /><div className="rounded-xl bg-surface-soft p-4 text-sm"><strong className="text-success">Verificación mock</strong><p className="mt-1 text-muted">No guardaremos datos bancarios reales.</p></div><SubmitButton label="Verificar y continuar" /></form></>}
      {step === "3" && <><Bell size={32} className="text-accent" weight="fill" /><h1 className="mt-4 text-3xl font-semibold tracking-[-0.04em]">No te pierdas ningún tip</h1><p className="mt-2 mb-7 text-muted">Te avisaremos en cuanto recibas un pago confirmado.</p><PushSetup vapidPublicKey={env.NEXT_PUBLIC_VAPID_PUBLIC_KEY} /><Link href="/onboarding?step=4" className="mt-5 flex min-h-12 items-center justify-center font-semibold text-muted">Continuar por ahora <ArrowRight className="ml-2" /></Link></>}
